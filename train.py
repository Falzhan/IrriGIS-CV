...
def train(config, save_dir=None, progress_callback=None):
    """
    ...
    try:
        ...
        if save_dir:
            checkpoint_dir = Path(save_dir) / f"checkpoints_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            log_dir = Path(save_dir) / f"logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        else:
            checkpoint_dir = Path("models") / f"checkpoints_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        if progress_callback:
            progress_callback("Created output directories", 5)
